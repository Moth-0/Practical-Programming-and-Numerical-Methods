...
struct vector {
	...
	vector& add(long double x) {
        data.push_back(x);
        return SELF;} 

	vector& push_back(long double x) {
        data.push_back(x); 
        return SELF;} 
...
vector operator*(const vector& v, long double a) {
    vector r = v; 
    r *= a; 
    return r;} 

vector operator*(long double a, const vector& v) {
    vector r = v; 
    r *= a; 
    return r; }
...
struct matrix {
    ...
    // Indexing
	long double get (size_t i, size_t j) {return cols[j][i];}
	void set(size_t i, size_t j, long double value){cols[j][i] = value;}
...
matrix operator/(const matrix& A,long double x){
	matrix R=A;
	R/=x;
	return R; }
...
matrix operator*(long double x,const matrix& A){
	return A*x; }
